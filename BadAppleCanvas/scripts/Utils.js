class KeyboardManager {
  constructor() {
    ...
    this.onKeyPressListener = key => {}
    this.onKeyUpListener = key => {}

    document.addEventListener(
      "keydown", this.onKeyPress.bind(this), false
    )
    document.addEventListener(
  ...
  setOnKeyPressListener = f => { this.onKeyPressListener = f }
  setOnKeyUpListener = f => { this.onKeyUpListener = f }
...
let RGB = (r, g, b) => `rgb(${r|0}, ${g|0}, ${b|0})`
let int = Number.parseInt
let rand = Math.random

export { MouseManager, KeyboardManager }
export { RGB, int, rand }